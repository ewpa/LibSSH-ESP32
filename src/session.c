    ...
    case SSH_PUBLICKEY_HASH_SHA1:
        {
            ...
            rc = sha1_update_esp32_port(ctx, ssh_string_data(blob), ssh_string_len(blob));
            if (rc != SSH_OK) {
                free(h);
                sha1_ctx_free(ctx);
                goto out;
    ...
    case SSH_PUBLICKEY_HASH_MD5:
        {
            ...
            rc = md5_update_esp32_port(ctx, ssh_string_data(blob), ssh_string_len(blob));
            if (rc != SSH_OK) {
                free(h);
                md5_ctx_free(ctx);
                goto out;
